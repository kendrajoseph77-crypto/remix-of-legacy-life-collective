    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-20 pb-4 relative overflow-hidden">
        <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 py-3 bg-white">
          <Link to="/">
            <img src={logoF} alt="5050 Fast logo" className="w-auto" style={{ height: "54px" }} />
          </Link>
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="text-center">
            <p className="text-base tracking-[0.3em] uppercase font-medium mb-6" style={{ color: BLUE }}>Cooperative Crowdfunding</p>
            <h1 className="text-4xl md:text-5xl font-bold mb-4 leading-tight">5050 Fast Works...</h1>
            ...
            <div className="flex items-center justify-center gap-6 mb-8">
              ...
              <Link
                ...
                style={{ background: `linear-gradient(180deg, ${BLUE}, hsl(224 85% 42%))` }}
              >
                Join Us Now
              </Link>
            </div>
              ...
              <div key={i} className="relative">
                ...
                <div className="rounded-xl p-6 bg-background border border-border">
                  <div className="flex items-baseline gap-3 mb-1">
                    ...
                    <span className="text-5xl font-bold uppercase" style={{ color: BLUE }}>{step.keyword}</span>
                  </div>
      ...
      <section className="py-24 relative overflow-hidden bg-card border-y border-border">
        <div className="absolute inset-0" style={{ background: `radial-gradient(ellipse 60% 50% at 50% 40%, hsl(224 85% 58% / 0.06) 0%, transparent 70%)` }} />
        <div className="max-w-5xl mx-auto px-6 relative z-10">
          <div className="text-center mb-10">
            ...
            <div className="w-16 h-[1px] mx-auto mb-12" style={{ background: `linear-gradient(90deg, transparent, ${BLUE}, transparent)` }} />
          </div>
          <TwoRingWheelhouse />
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-2xl mx-auto">
          {automations.map((item, i) => (
            <div key={i} className="flex items-center gap-3 p-4 rounded-lg bg-card border border-border">
              <CheckCircle size={16} style={{ color: BLUE }} className="flex-shrink-0" />
      ...
      <section className="py-14 bg-card border-y border-border">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="flex justify-center mt-10">
            <div className="rounded-xl border-2 p-3 md:p-4 bg-background max-w-sm w-full" style={{ borderColor: fastLevel.color }}>
              <div className="relative mb-3">
                <img src={fastLevel.logo} alt={`${fastLevel.name} logo`} className="absolute left-0 top-1/2 -translate-y-1/2 w-auto" style={{ height: "48px" }} />
                <h3 className="text-xl font-bold text-center" style={{ color: fastLevel.color }}>{fastLevel.name}</h3>
              </div>
              <div className="grid grid-cols-3 gap-1.5 mb-4">
                {fastLevel.vaults.map((v, i) => (
                  <div key={i} className="p-1.5 rounded-lg border border-border bg-muted/20 flex flex-col items-center justify-center">
                    <p className="text-xs font-semibold truncate w-full text-center" style={{ color: fastLevel.color }}>{v.contribution}</p>
              ...
              <p className="text-center">
                ...
                <span className="text-xl font-bold" style={{ color: fastLevel.color }}>{fastLevel.total}</span>
          ...
          <div className="text-center mt-14">
            <h3 className="text-2xl md:text-3xl font-bold mb-2" style={{ color: BLUE }}>
